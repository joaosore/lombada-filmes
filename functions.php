...
function html5blank_styles()
{
    ...
    wp_register_style('johnnydev', get_template_directory_uri() . '/dist/main.css', array(), '1.0', 'all');
    wp_enqueue_style('johnnydev'); // Enqueue it!
}
...
function jd_component($name, $function = null) {
    include get_template_directory() . '/src/components/' . $name . '/index.php';

    if($function) {
        $function = (object) $function;
        return call_user_func_array($name, [$function->data]);
    }

}

//Cria o Menu Academia Medway
function create_trabalhos_post_types() {
    register_post_type( 'trabalhos',
        array(
            'labels' => array(
                'name' => __( 'Trabalhos' ),
                'singular_name' => __( 'Trabalhos' )
            ),
            'public' => true,
            'hierarchical' => true,
            'show_ui' => true,
  ...
  add_action( 'init', 'create_trabalhos_post_types' );
...
function filter_post_contato() {
	...
	if ($_POST)
	{
		...
		// {
			...
			$response['mensagem'] ='Mensagem envida com sucesso';
		// }
		// else
		// {
		// 	$response['status'] = 'Erro';
		// 	$response['mensagem'] = 'Erro ao enviar mensagem';
		// }	
	}
